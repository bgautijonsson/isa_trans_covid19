...
p <- plot_dat %>% 
  filter(par == "Beta", country != "United States") %>% 
  mutate(location = fct_reorder(location, estimate)) %>% 
  ggplot(aes(location, estimate, ymin = conf.low, ymax = conf.high,
             text = str_c("Location: ", location, "<br>",
                          "Median: ", round(estimate, 4), "<br>",
...
p <- plot_dat %>% 
  filter(par == "Beta_tilde", country != "United States") %>% 
  mutate(location = fct_reorder(location, estimate)) %>% 
  ggplot(aes(location, estimate, ymin = conf.low, ymax = conf.high,
             text = str_c("Location: ", location, "<br>",
                          "Median: ", round(estimate, 4), "<br>",
  ...
  coord_flip() +
  scale_y_log10() +
  labs(title = "Beta: Location-Specific Estimates",
       y = "Posterior median and 95% PI") +
  theme(axis.title.y = element_blank())
...
p <- plot_dat %>% 
  filter(par == "Nu", country != "United States") %>% 
  mutate(location = fct_reorder(location, estimate)) %>% 
  ggplot(aes(location, estimate, ymin = conf.low, ymax = conf.high,
             text = str_c("Location: ", location, "<br>",
                          "Median: ", round(estimate, 4), "<br>",
...
```{r nu_Z, eval = F, include = F}
p <- spread_draws(m, log_nu[id], mu_nu, sigma_nu) %>%
  mutate(z = (log_nu - mu_nu) / sigma_nu) %>% 
  select(id, z) %>% 
  inner_join(locations, by = c("id" = "location_id")) %>% 
  group_by(location) %>% 
  summarise(estimate = median(z),
            conf.low = quantile(z, 0.025), 
            conf.high = quantile(z, 0.975)) %>% 
  ungroup %>% 
  mutate(location = fct_reorder(location, estimate)) %>% 
  ggplot(aes(location, estimate, ymin = conf.low, ymax = conf.high,
             text = str_c("Location: ", location, "<br>",
...
spread_draws(m, nu[id]) %>% 
  ...
  ungroup %>% 
  inner_join(locations, by = c("id" = "location_id")) %>% 
  filter(location %in% c("Sudan", "Ukraine", "Algeria", "Azerbaijan", "Armenia",
                         "Argentina", "Bangladesh", "Bahrain", "Afghanistan",
                         "Bolivia", "Brazil", "Sweden", "Belarus", "United Arab Emirates",
                         "Switzerland", "Austria", "Belgium", "Turkey", 
  ...
  ggplot(aes(location, estimate, ymin = conf.low, ymax = conf.high,
             text = str_c("Location: ", location, "<br>",
                          ...
                          "Lower: ", round(conf.low, 4), "<br>",
                          "Upper: ", round(conf.high, 4)))) +
  geom_hline(yintercept = exp(-1), lty = 2, alpha = 0.7) +
  geom_linerange() +
  geom_point() +
  coord_flip(ylim = c(0 , 1)) +
  scale_y_continuous(breaks = c(exp(-1), 0.5, 0.8, 0.1),
  ...
  labs(y = "Posterior median and 95% PI") +
  theme(axis.title.y = element_blank()) +
  ggsave(str_c(params$date, "/Figures/inflection_points.png"),
         width = 6, height = 0.4 * 6, scale = 2)
```
...
plot_dat %>% 
  ...
  labs(title = "S: Location-Specific Estimates",
       subtitle = "Posterior median and 95% PI") +
  theme(axis.title = element_blank()) +
  ggsave(str_c(params$date, "/Figures/S.png"),
         width = 6, height = 0.625 * 6, scale = 2.5)
```
...
results %>% 
  ...
  scale_y_log10(breaks = c(1e3, 1e4, 1e5, 1e6, 1e7),
                labels = label_comma()) +
  theme(axis.title = element_blank()) +
  labs(title = "Final number of cases: Location-Specific Estimates",
       subtitle = "Posterior median and 95% PI") +
  ggsave(str_c(params$date, "/Figures/max_cases.png"),
         width = 6, height = 0.625 * 6, scale = 2.5)
```
...
p <- plot_dat %>% 
  filter(par == "Phi", country != "United States") %>% 
  mutate(location = fct_reorder(location, estimate)) %>% 
  ggplot(aes(location, estimate, ymin = conf.low, ymax = conf.high, 
             text = str_c("Location: ", location, "<br>",
                          "Median: ", round(estimate, 4), "<br>",
...
```{r asymptote_WORLD_save, fig.width = 10, fig.asp = 0.4, eval = F}
results %>% 
  ungroup %>% 
  filter(par %in% c("S")) %>% 
  mutate(par = str_replace(par, "phi_inv", "phi")) %>% 
  ...
  coord_flip() +
  scale_y_log10(limits = c(1e4, NA), 
                breaks = c(1e4, 1e5, 1e6, 1e7),
                labels = label_comma()) +
  theme(axis.title = element_blank()) +
  ggsave(str_c(params$date, "/Figures/asymptote_world.png"),
         width = 6, height = 0.4 * 6, scale = 2)
```
...
```{r, fig.asp=0.4, eval = FALSE}
spread_draws(m, alpha[id], beta[id], nu[id], S[id]) %>% 
  mutate(days = alpha + log((0.1)^(-nu) - 1) / beta - log(nu)) %>% 
  arrange(id, days) %>% 
  group_by(id) %>% 
  mutate(q = row_number() / n()) %>% 
  ungroup %>% 
  inner_join(locations, by = c("id" = "location_id")) %>% 
  filter(location %in% c("Brazil",  "Indonesia", "Colombia", "Russia", "Mexico"), 
         is.finite(days)) %>% 
  mutate(end_date = start_date + days) %>% 
  filter(end_date <= ymd("2022-01-01")) %>% 
  group_by(location) %>% 
  mutate(lab_x = end_date,
         lab_x = ifelse(end_date == max(end_date), end_date, NA_POSIXct_) %>% as_date,
         lab_y = ifelse(is.na(lab_x), NA_real_, q),
         lab_x = case_when(location == "Colombia" ~ lab_x + 24,
                           location == "Indonesia" ~ lab_x + 24,
                           location == "Brazil" ~ lab_x + 16,
                           TRUE ~ lab_x + 18.5),
         lab_y = case_when(location == "Indonesia" ~ lab_y - 0.017,
                           location == "Brazil" ~ lab_y + 0.006, 
                           location == "Colombia" ~ lab_y - 0.008,
                           location == "Russia" ~ lab_y + 0.018,
                           TRUE ~ lab_y),
         label = location) %>% 
  ggplot(aes(end_date, q, group = location, col = location)) +
  geom_line() +
  geom_text(aes(x = lab_x, y = lab_y, label = label)) +
  scale_x_date(breaks = ymd(c("2020-07-01",
                              "2021-01-01", "2021-07-01", 
                              "2022-01-01", "2023-01-01", "2024-01-01")),
               date_labels = "%B %Y") +
  scale_y_continuous(labels = label_percent(), expand = expansion(mult = 0.005), limits = c(0, 1.03)) +
  scale_colour_brewer(type = "qual", palette = "Set1") +
  coord_cartesian(xlim = ymd(c("2020-07-01", "2022-02-01"))) +
  # ggtitle("Cumulative probability of first wave ending",
  #         subtitle = "First wave ended if total cases reach 90% of asymptote") +
  theme(axis.title = element_blank()) +
  ggsave(str_c(params$date, "/Figures/first_wave_ending.png"),
         width = 6, height = 0.4 * 6, scale = 1.6)
```
...
tidy(m, pars = "corr_mat", conf.int = T, rhat = T, ess = T) %>% 
  ...
  select(par2, estimate, conf.low, conf.high, rhat, ess) %>% 
  set_names(c("Parameter", "Median", "Lower", "Upper", "Rhat", "ESS")) %>%
  kable(escape = FALSE, format = "latex", booktabs = TRUE, digits = 2,
        caption = "Caption",
        align = c("l", rep("c", ncol(.) - 1))) %>% 
  kable_styling(latex_options = "HOLD_position", full_width = TRUE) %>% 
  group_rows(index = c(
    "$\\boldsymbol{\\ln(\\alpha)}$" = 3,
    "$\\boldsymbol{\\ln(\\beta)}$" = 2,
    "$\\boldsymbol{\\ln(\\nu)}$" = 1
  ),latex_align = "l", latex_gap_space = "1", escape = F) %>% 
  add_header_above(c("", "", "95% PI" = 2, "", "")) -> tab
```



```{r latex_table, eval = F, results="asis"}
results %>% 
  ungroup %>% 
  filter(par %in% c("mu_beta", "sigma_beta", 
                    "mu_alpha", "sigma_alpha", "nu_alpha", 
  ...
  add_header_above(c("", "", "95% PI" = 2, "Convergence" = 2)) -> tab
```
